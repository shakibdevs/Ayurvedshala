...
import { 
  ...
  Settings, Leaf, Trash2, Database, UserPlus, AlertCircle, ChevronRight, Wifi, Clock, Eye, X, Upload, Package, History, HeartPulse, ScrollText, CheckSquare, Target, Layers, Box, Cloud
} from 'lucide-react';
...
const Sidebar = ({ user, view, setView, onLogout, settings, isVercel }) => (
  <div className="w-64 bg-emerald-900 text-white flex flex-col p-4 shadow-xl shrink-0">
    ...
    <div className="mt-auto">
        <div className="text-[10px] text-emerald-400/60 mb-2 text-center flex items-center justify-center gap-1">
            {isVercel ? <><Cloud size={10} /> Cloud Connected</> : <><Wifi size={10} /> Local Network</>}
        </div>
        <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 text-emerald-200 hover:text-white p-2 border border-emerald-800 rounded hover:bg-emerald-800 transition-colors"><LogOut size={16}/> Logout</button>
    </div>
  </div>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
            <div className="md:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-emerald-100 h-full flex flex-col">
                ...
                <form onSubmit={(e) => { e.preventDefault(); onAddPatient(newPatient); setNewPatient({ name: '', age: '', gender: 'Male', phone: '', prakriti: 'Vata' }); }} className="space-y-3 flex-1 flex flex-col">
                    <input placeholder="Name" className="w-full p-3 border rounded-lg" value={newPatient.name} onChange={e=>setNewPatient({...newPatient, name:e.target.value})} required/>
                    <div className="flex gap-2"><input placeholder="Age" className="w-full p-3 border rounded-lg" value={newPatient.age} onChange={e=>setNewPatient({...newPatient, age:e.target.value})}/><select className="p-3 border rounded-lg" value={newPatient.gender} onChange={e=>setNewPatient({...newPatient, gender:e.target.value})}><option>Male</option><option>Female</option></select></div>
                    <input placeholder="Phone" className="w-full p-3 border rounded-lg" value={newPatient.phone} onChange={e=>setNewPatient({...newPatient, phone:e.target.value})}/>
            ...
            <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-emerald-100 h-full flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="relative"><Search className="absolute left-2 top-2.5 text-slate-400" size={16}/><input className="pl-9 p-2 border rounded-lg text-sm w-64 focus:ring-1 focus:ring-emerald-500 outline-none" placeholder="Search..." value={search} onChange={(e)=>setSearch(e.target.value)}/></div>
                </div>
            ...
            <div className="grid grid-cols-12 gap-6 flex-1 overflow-hidden">
                <div className="col-span-4 flex flex-col space-y-6 overflow-y-auto pr-2">
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                        ...
                        <div className="mb-4"><label className="text-xs font-bold text-slate-500 uppercase">Chief Complaints</label><div className="flex gap-2 mb-2"><AutoSuggest items={masters.symptoms} value={currentSymptom} onChange={setCurrentSymptom} placeholder="Add symptom..."/><button onClick={()=>{if(currentSymptom){setRxForm({...rxForm, symptoms:[...rxForm.symptoms, currentSymptom]});setCurrentSymptom('')}}} className="bg-emerald-100 text-emerald-800 p-2 rounded"><Plus size={18}/></button></div><div className="flex flex-wrap gap-2">{rxForm.symptoms.map((s,i)=>(<span key={i} className="bg-slate-100 text-slate-700 text-xs px-2 py-1 rounded flex items-center gap-1">{s} <button onClick={()=>setRxForm({...rxForm, symptoms: rxForm.symptoms.filter((_,idx)=>idx!==i)})}><X size={12}/></button></span>))}</div></div>
                        <div className="grid grid-cols-3 gap-2 mb-4"><div><label className="text-[10px] font-bold uppercase">Nadi</label><input className="w-full p-1 border rounded text-sm" value={rxForm.nadi} onChange={e=>setRxForm({...rxForm, nadi:e.target.value})}/></div><div><label className="text-[10px] font-bold uppercase">Agni</label><input className="w-full p-1 border rounded text-sm" value={rxForm.agni} onChange={e=>setRxForm({...rxForm, agni:e.target.value})}/></div><div><label className="text-[10px] font-bold uppercase">Koshta</label><input className="w-full p-1 border rounded text-sm" value={rxForm.koshta} onChange={e=>setRxForm({...rxForm, koshta:e.target.value})}/></div></div>
                    ...
                    <div className="bg-emerald-50 p-5 rounded-xl border border-emerald-100"><label className="text-xs font-bold text-emerald-800 flex items-center gap-2"><DollarSign size={14}/> Internal Charges</label><input type="number" className="w-full p-2 mt-2 border border-emerald-300 rounded font-mono text-lg" value={rxForm.internalCharges} onChange={e=>setRxForm({...rxForm, internalCharges:e.target.value})} placeholder="₹ 0.00"/></div>
                </div>
                <div className="col-span-8 flex flex-col space-y-6 overflow-y-auto pr-2">
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                        ...
                        {medEntryMode === 'packed' && (
                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 mb-4 space-y-3">
                                ...
                                <div className="grid grid-cols-4 gap-2"><div><input list="doseOptions" placeholder="Dose" className="p-2 border rounded w-full" value={currentMed.dose} onChange={e=>setCurrentMed({...currentMed, dose:e.target.value})}/></div><div><input list="freqOptions" placeholder="Freq" className="p-2 border rounded w-full" value={currentMed.freq} onChange={e=>setCurrentMed({...currentMed, freq:e.target.value})}/></div><div><input list="durOptions" placeholder="Duration" className="p-2 border rounded w-full" value={currentMed.duration} onChange={e=>setCurrentMed({...currentMed, duration:e.target.value})}/></div><button onClick={addPackedToRx} className="bg-emerald-600 text-white rounded font-bold hover:bg-emerald-700">Add</button></div>
                            </div>
                        )}
                        {medEntryMode === 'combination' && (
                            <div className="bg-amber-50 p-3 rounded-lg border border-amber-200 mb-4 space-y-3">
                                <div className="flex gap-2 items-center"><Layers size={16} className="text-amber-700"/><input className="font-bold p-2 bg-white border border-amber-300 rounded flex-1" placeholder="Combination Name (e.g. Vata Shamak Yoga)" value={currentComb.name} onChange={e=>setCurrentComb({...currentComb, name:e.target.value})}/></div>
                                <div className="flex gap-2 bg-white p-2 rounded border border-amber-100"><div className="flex-1"><AutoSuggest label="" items={masters.medicines} value={currentIngredient.name} onChange={v=>setCurrentIngredient({...currentIngredient, name:v})} placeholder="Ingredient..."/></div><input className="w-20 p-2 border rounded" placeholder="Qty" value={currentIngredient.amount} onChange={e=>setCurrentIngredient({...currentIngredient, amount:e.target.value})}/><select className="p-2 border rounded bg-white" value={currentIngredient.unit} onChange={e=>setCurrentIngredient({...currentIngredient, unit:e.target.value})}>{PRESET_UNITS.map(u=><option key={u}>{u}</option>)}</select><button onClick={addIngredientToComb} className="bg-amber-600 text-white px-3 rounded"><Plus size={16}/></button></div>
                                {currentComb.ingredients.length > 0 && (<div className="flex flex-wrap gap-2">{currentComb.ingredients.map((ing, i) => (<span key={i} className="text-xs bg-white border border-amber-200 text-amber-900 px-2 py-1 rounded flex items-center gap-1">{ing.name} ({ing.amount}{ing.unit}) <button onClick={()=>setCurrentComb({...currentComb, ingredients:currentComb.ingredients.filter((_,idx)=>idx!==i)})}>&times;</button></span>))}</div>)}
                                <div className="grid grid-cols-4 gap-2 pt-2 border-t border-amber-200"><div><input list="doseOptions" placeholder="Total Dose" className="p-2 border rounded w-full" value={currentComb.dose} onChange={e=>setCurrentComb({...currentComb, dose:e.target.value})}/></div><div><input list="freqOptions" placeholder="Freq" className="p-2 border rounded w-full" value={currentComb.freq} onChange={e=>setCurrentComb({...currentComb, freq:e.target.value})}/></div><div><input list="durOptions" placeholder="Dur" className="p-2 border rounded w-full" value={currentComb.duration} onChange={e=>setCurrentComb({...currentComb, duration:e.target.value})}/></div><div><input placeholder="Anupan" className="p-2 border rounded w-full" value={currentComb.anupan} onChange={e=>setCurrentComb({...currentComb, anupan:e.target.value})}/></div></div>
                                <button onClick={addCombinationToRx} className="w-full bg-amber-700 text-white py-2 rounded font-bold hover:bg-amber-800">Add Combination</button>
                            </div>
                        )}
                        ...
                        <div className="space-y-2">
                            {rxForm.medicines.map((m,i)=>(
                                <div key={i} className={`flex justify-between items-center p-3 border rounded hover:bg-slate-50 ${m.type==='combination'?'bg-amber-50 border-amber-200':'bg-white'}`}>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            {m.type==='combination' ? <Layers size={14} className="text-amber-700"/> : <Box size={14} className="text-slate-500"/>}
                                            <span className={`font-bold ${m.type==='combination'?'text-amber-900':'text-teal-800'}`}>{m.name}</span>
                                        </div>
                    ...
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4"><div className="col-span-2"><AutoSuggest label="Pathya / Apathya" items={masters.pathyas} value={rxForm.pathya} onChange={v=>setRxForm({...rxForm, pathya:v})} placeholder="Diet..."/></div><div><label className="text-xs font-bold text-slate-500 uppercase">Follow Up</label><input type="text" className="w-full p-2 border rounded" value={rxForm.followUp} onChange={e=>setRxForm({...rxForm, followUp:e.target.value})} placeholder="Date or Duration"/></div><div><label className="text-xs font-bold text-slate-500 uppercase">Notes</label><input className="w-full p-2 border rounded" value={rxForm.notes} onChange={e=>setRxForm({...rxForm, notes:e.target.value})} placeholder="Rest..."/></div></div>
            ...
            <div className="bg-white p-6 rounded-b-xl rounded-tr-xl shadow-sm border border-emerald-200 space-y-4 flex-1 flex flex-col overflow-hidden">
                <div className="flex justify-between items-end bg-emerald-50 p-4 rounded-lg shrink-0">
                    <div className="flex gap-2 items-end w-full">
                        <div className="flex-1"><label className="text-xs font-bold text-emerald-800 uppercase">New {activeTab.slice(0,-1)} Name</label><input className="w-full p-2 border rounded" value={newItem.name} onChange={e=>setNewItem({...newItem, name:e.target.value})} placeholder="Name..."/></div>
                        {activeTab === 'medicines' && (<><div className="w-32"><label className="text-xs font-bold text-emerald-800 uppercase">Type</label><input className="w-full p-2 border rounded" value={newItem.type} onChange={e=>setNewItem({...newItem, type:e.target.value})} placeholder="Vati/Churna"/></div><div className="w-24"><label className="text-xs font-bold text-emerald-800 uppercase">Stock</label><input type="number" className="w-full p-2 border rounded" value={newItem.stock} onChange={e=>setNewItem({...newItem, stock:parseInt(e.target.value)||0})}/></div></>)}
...
const StaffDashboard = ({ prescriptions, onView, onPrint }) => {
    ...
    return (
        <div className="space-y-6 h-full flex flex-col">
            <div className="flex justify-between items-end shrink-0"><h2 className="text-2xl font-bold flex items-center gap-2"><Pill/> Pharmacy Queue</h2><div className="relative"><Search className="absolute left-3 top-3 text-slate-400" size={16}/><input className="pl-9 p-2 border rounded-lg w-64" placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)}/></div></div>
            <div className="grid gap-4 overflow-y-auto flex-1 content-start">{filteredRx.map((rx) => (<div key={rx._id} className="bg-white p-4 rounded-xl shadow-sm border flex justify-between items-center relative overflow-hidden"><div><h3 className="font-bold text-lg">{rx.patientName}</h3><p className="text-sm text-teal-600 font-bold">{rx.diagnosis}</p><p className="text-xs text-slate-400">{new Date(rx.date).toLocaleString()}</p></div><div className="text-right flex gap-3 items-center"><div><p className="text-xs font-bold text-emerald-700 uppercase">Collect</p><p className="text-xl font-bold">₹{rx.internalCharges}</p></div><button onClick={()=>onView(rx)} className="p-3 bg-slate-100 rounded-lg"><Eye size={20}/></button><button onClick={()=>onPrint(rx)} className="p-3 bg-teal-600 text-white rounded-lg"><Printer size={20}/></button></div></div>))}</div>
        </div>
    );
};
...
const AutoSuggest = ({ label, items, value, onChange, placeholder, displayKey = 'name' }) => {
  ...
  return (
    <div className="relative group w-full">
      ...
      <input value={value} onChange={(e) => { onChange(e.target.value); setShow(true); }} onFocus={() => setShow(true)} className="w-full p-2 border border-slate-200 rounded focus:ring-2 focus:ring-emerald-500 outline-none" placeholder={placeholder} />
...
const Login = ({ onLogin, settings, serverIp, setServerIp, isVercel }) => {
    ...
    <div className="h-screen w-full bg-emerald-50 font-sans text-slate-900 flex items-center justify-center">
      <div className="bg-white shadow-2xl w-full h-full flex overflow-hidden">
        <div className="hidden md:flex flex-col justify-center items-center w-1/2 bg-gradient-to-br from-emerald-700 to-emerald-900 text-white p-12 relative">
          <div className="w-48 h-48 bg-white/10 rounded-full flex items-center justify-center mb-8 backdrop-blur-sm p-4 overflow-hidden shadow-inner">{settings.logoUrl ? <img src={settings.logoUrl} className="w-full h-full object-contain rounded-full" /> : <Leaf className="w-24 h-24 text-emerald-100" />}</div>
          <h1 className="text-5xl font-bold mb-4 text-center font-serif tracking-wide">{settings.clinicName || 'Ayurvedshala'}</h1>
          <div className="absolute bottom-8 text-sm text-emerald-300/50 flex items-center gap-2">
              {isVercel ? <><Cloud size={16} /> Server: Cloud (Secure)</> : <><Wifi size={16} /> Server: {serverIp}</>}
          </div>
        </div>
        <div className="w-full md:w-1/2 p-20 flex flex-col justify-center bg-white">
          {showIp && !isVercel ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); const target = role === 'doctor' ? (settings?.doctorPwd || 'admin') : (settings?.staffPwd || 'staff'); if (password === target) onLogin({ role, name: role === 'doctor' ? (settings?.doctorName || 'Dr. Dimple') : 'Staff' }); else alert('Invalid Code'); }} className="space-y-8 w-full max-w-md mx-auto">
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-5 bg-slate-50 border rounded-xl text-lg outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Password" />
                ...
                {!isVercel && <div className="text-center pt-4"><button type="button" onClick={() => setShowIp(true)} className="text-sm text-slate-400 hover:text-emerald-600 flex items-center justify-center gap-2 mx-auto"><Wifi size={16} /> Configure Connection</button></div>}
            </form>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const newApiUrl = checkVercel ? '/api' : `http://${serverIp}:5000/api`;
    setApiUrl(newApiUrl);
    localStorage.setItem('ayur_server_ip', serverIp);
  }, [serverIp]);
  ...
  if (!user) return <Login onLogin={setUser} settings={settings} serverIp={serverIp} setServerIp={setServerIp} isVercel={isVercel} />;

  return (
    <div className="flex h-screen bg-emerald-50 font-sans text-slate-800 overflow-hidden">
      <Sidebar user={user} view={view} setView={setView} onLogout={()=>setUser(null)} settings={settings} isVercel={isVercel} />
        ...
        {viewRx && (
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl overflow-hidden">
                    ...
                    <div className="p-6 space-y-4"><div className="flex justify-between border-b pb-4"><div><p className="text-xs text-slate-400">Patient</p><p className="font-bold">{viewRx.patientName}</p></div></div><div><h4 className="font-bold mb-2 text-sm text-slate-600">Medicines</h4><ul className="space-y-2">{viewRx.medicines.map((m,i)=>(<li key={i} className="text-sm border p-2 rounded flex justify-between flex-col"><div className="flex justify-between w-full font-bold"><span>{m.name}</span><span className="text-slate-500 font-normal">{m.dose} • {m.freq}</span></div>{m.type==='combination' && m.ingredients && <div className="text-xs text-amber-700 mt-1 bg-amber-50 p-1 rounded"><strong>Mix:</strong> {m.ingredients.map(ing=>`${ing.name} ${ing.amount}${ing.unit}`).join(' + ')}</div>}</li>))}</ul></div></div>
                </div>
            </div>
        )}
      </div>
